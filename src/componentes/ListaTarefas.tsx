...
// Importa os componentes FlatList, Text, Box da biblioteca NativeBase
...
const TarefaItem: React.FC<TarefaItemProps> = ({ id, titulo }) => {
  ...
  return (
    <Box
      ...
      bg="#1A5276" // Cor de fundo (azul escuro)
      p={4} // Padding interno
    ...
    >
      {/* Modo de exibição */}
      <Text flex={3} fontSize={18} color="white">{titulo}</Text> {/* Exibe o título da tarefa */}
    </Box>
  );
};
...
const ListaTarefas: React.FC = () => {
  ...
  return (
    <FlatList
      ...
      style={{ width: '100%', backgroundColor: '#1A5276' }} // Largura da lista e cor de fundo
    />
  );
};
